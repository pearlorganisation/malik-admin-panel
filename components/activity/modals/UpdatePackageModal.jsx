...
import { useUpdatePackageMutation } from '@/features/activity/activityApi';
import {
  Plus,
  Trash2,
  X,
...
export default function UpdatePackageModal({ packageData, isOpen, onClose, onSuccess }) {
  const [updatePackage, { isLoading }] = useUpdatePackageMutation();
  const [errors, setErrors] = useState({});

  // Initialize form with existing packageData
  const [formData, setFormData] = useState({
    name: packageData?.name || '',
    description: packageData?.description || '',
    price: packageData?.price || '',
    whatInclude: packageData?.whatInclude?.length ? packageData.whatInclude : [''],
    whatExclude: packageData?.whatExclude?.length ? packageData.whatExclude : [''],
    bookingFields: packageData?.bookingFields || [],
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        isActive: packageData.isActive, // retain existing status
      };
      ...
      if (backendErrors && Array.isArray(backendErrors)) {
        ...
        setErrors({
          submit: err?.data?.message || err?.message || "Failed to update package",
        });
      }
    }
  };
      ...
      {formData[field].map((item, index) => (
        <div key={index} className="flex gap-2">
          <input
            ...
            className="flex-1 input border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
          />
          <button
            type="button"
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* HEADER */}
        <div className="bg-linear-to-r from-blue-600 to-indigo-600 px-6 py-4 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2 text-white">
            <PackageIcon className="w-6 h-6" />
            <h2 className="text-xl font-bold">Update Activity Package</h2>
          </div>
          <button
            type="button"
            onClick={() => onClose?.()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full input border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none ${
            ...
            <div className="h-full">
              ...
              <textarea
                ...
                className={`w-full h-[calc(100%-1.75rem)] input border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none ${
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end">
              ...
              <button
                ...
                className="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-blue-100 flex items-center gap-1 transition-colors"
              >
                ...
                >
                  ...
                  <div className="col-span-12 sm:col-span-3">
                    ...
                    <input
                      ...
                      className={`w-full input border rounded px-2 py-1.5 text-sm focus:ring-1 focus:ring-blue-500 focus:outline-none ${
                  ...
                  <div className="col-span-6 sm:col-span-1">
                    ...
                    <input
                      ...
                      className="w-full input border border-gray-300 rounded px-2 py-1.5 text-sm focus:ring-1 focus:ring-blue-500 focus:outline-none"
                    />
        ...
        <div className="border-t bg-gray-50 px-6 py-4 flex justify-end gap-3 shrink-0">
          ...
          <button
            ...
            disabled={isLoading}
            className="px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 disabled:opacity-70 disabled:cursor-not-allowed shadow-sm transition-all flex items-center gap-2"
          >
            {isLoading ? (
              <>
